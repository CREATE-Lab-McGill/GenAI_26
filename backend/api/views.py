...
def question_alternative(request, pk):
    try:
        question = Question.objects.get(pk=pk)
        q_data = QuestionSerializer(question).data
        alternative = generate_alternative_question(q_data)

        question.prompt = alternative.get("prompt", question.prompt)
    ...
    try:
        ...
        with open(docx_path, "rb") as doc_file:
            ...
            response["Content-Disposition"] = (
                f'attachment; filename="{set_name}-{mode}.docx"'
            )
...
def reorder_questions(request, pk):
    try:
        ...
        questions = {
            ...
            for question in problem_set.questions.all()
        }

        for index, question_id in enumerate(ordered_ids):
            if question_id in questions:
                questions[question_id].order = index
                questions[question_id].save(update_fields=["order"])

        return Response(
            QuestionSerializer(
                problem_set.questions.all(),
                many=True,
            ).data
        )
